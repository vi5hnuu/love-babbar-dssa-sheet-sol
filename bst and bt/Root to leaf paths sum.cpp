...
void treePathsSum(Node *root,long long num,long long &sm){
    if(!root){
        return;
    }
    ...
    treePathsSum(root->left,num*10+root->data,sm);
    treePathsSum(root->right,num*10+root->data,sm);
}

long long treePathsSum(Node *root)
{
    long long sm=0;
    treePathsSum(root,0,sm);
    return sm;
}
