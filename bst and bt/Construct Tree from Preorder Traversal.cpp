...
Node *constructTree(int pre[],int &idx,char preLN[]){
    ...
    root->left=constructTree(pre,idx,preLN);
    root->right=constructTree(pre,idx,preLN);
    return root;
}
Node *constructTree(int n, int pre[], char preLN[]){
    int idx=0;
    return constructTree(pre,idx,preLN);
}
