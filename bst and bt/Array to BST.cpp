...
class Solution {
private:
    void preorder(vector<int> &po,vector<int>& nums,int low,int high){
        if(low>high){
            return;
        }
        int mid=(low+high)/2;
        po.push_back(nums[mid]);
        preorder(po,nums,low,mid-1);
        preorder(po,nums,mid+1,high);
    }
public:
    vector<int> sortedArrayToBST(vector<int>& nums) {
        vector<int> po;
        preorder(po,nums,0,nums.size()-1);
        return po;
    }
};