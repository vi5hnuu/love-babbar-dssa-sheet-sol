...
*************************************************************/
bool validateBST(BinaryTreeNode<int> *root,int limitLeft,int limitRight) {
    if(!root){
        return true;
    }
    if(!(root->data>=limitLeft && root->data<=limitRight)){
        return false;
    }
    if(!validateBST(root->left,limitLeft,root->data)){
        return false;
    }
    return validateBST(root->right,root->data,limitRight);
}
